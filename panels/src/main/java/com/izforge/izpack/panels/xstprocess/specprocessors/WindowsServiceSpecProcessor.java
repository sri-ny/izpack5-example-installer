...
public class WindowsServiceSpecProcessor extends SpecProcessor
{
  ...
  public com.izforge.izpack.panels.xstprocess.Processable processElement(IXMLElement element)
  {
    ...
    WindowsService.ServiceOperation serviceOpEnum = null;
    try {
      serviceOpEnum = WindowsService.ServiceOperation.valueOf(serviceOperation.toUpperCase());
    } catch (Exception e) {
      System.err.println("invalid service operation \"serviceOperation\" attribute for <windowsservice>: " + serviceOperation);
      return null;
    }
    
    return new com.izforge.izpack.panels.xstprocess.processables.WindowsService(serviceName, serviceOpEnum);
  }
}
